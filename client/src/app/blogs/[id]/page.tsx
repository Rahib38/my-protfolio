...
console.log(id)
  const res=await fetch(`http://localhost:5001/api/blogs/${id}`,{next:{tags:['blogs']}})
  const blogs=await res.json()
const blog=blogs?.data
  return (
    <div className="max-w-3xl mx-auto p-5">
    <h1 className="text-3xl font-bold  mb-4">{blog.title}</h1>
    <p className="text-sm  mb-4">Published on: {new Date(blog.createdAt).toLocaleDateString()}</p>
